...
$(document).ready(function() {
	@if (Route::currentRouteName() == 'admin.benefit.list')
	// Get list page data
	var getListDataUrl = "{{route('admin.benefit.list-request')}}";
	...
	var dTable = $('#list-data').on('init.dt', function () {$('#dataTableLoading').hide();}).DataTable({
		...
		@if ($isAllow || in_array('benefit.edit', $allowedRoutes) || in_array('benefit.delete', $allowedRoutes))
	...
	$(document).on('click', '.status', function() {
		...
		listActions('benefit', 'status', id, actionType, dTable);
	});
	...
	$(document).on('click', '.delete', function() {
		var id = $(this).data('id');
		var actionType 	= $(this).data('action-type');
		listActions('benefit', 'delete', id, actionType, dTable);
	});
	...
	$('.bulkAction').on('click', function() {
		...
		if(selectedIds.length > 0) {
			var actionType = $(this).data('action-type');
			bulkActions('benefit', 'bulk-actions', selectedIds, actionType, dTable);
		} else {
