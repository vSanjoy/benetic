...
$(document).ready(function() {
	@if (Route::currentRouteName() == 'admin.howItWork.list')
	// Get list page data
	var getListDataUrl = "{{route('admin.howItWork.list-request')}}";
	...
	var dTable = $('#list-data').on('init.dt', function () {$('#dataTableLoading').hide();}).DataTable({
		...
		columns: [
		@if ($isAllow || in_array('howItWork.change-status', $allowedRoutes) || in_array('howItWork.delete', $allowedRoutes))
			{
				data: 'id',
				orderable: false,
				searchable: false,
		...
		@if ($isAllow || in_array('howItWork.edit', $allowedRoutes) || in_array('howItWork.delete', $allowedRoutes))
	...
	$(document).on('click', '.status', function() {
		...
		listActions('howItWork', 'status', id, actionType, dTable);
	});
	...
	$(document).on('click', '.delete', function() {
		var id = $(this).data('id');
		var actionType 	= $(this).data('action-type');
		listActions('howItWork', 'delete', id, actionType, dTable);
	});
	...
	$('.bulkAction').on('click', function() {
		...
		if(selectedIds.length > 0) {
			var actionType = $(this).data('action-type');
			bulkActions('howItWork', 'bulk-actions', selectedIds, actionType, dTable);
		} else {
