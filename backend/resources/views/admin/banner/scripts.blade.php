...
$(document).ready(function() {
	@if (Route::currentRouteName() == 'admin.banner.list')
	// Get list page data
	var getListDataUrl = "{{route('admin.banner.list-request')}}";
	...
	var dTable = $('#list-data').on('init.dt', function () {$('#dataTableLoading').hide();}).DataTable({
		...
		columns: [
		@if ($isAllow || in_array('banner.change-status', $allowedRoutes) || in_array('banner.delete', $allowedRoutes))
			{
				data: 'id',
				orderable: false,
				searchable: false,
		...
		@if ($isAllow || in_array('banner.edit', $allowedRoutes) || in_array('banner.delete', $allowedRoutes))
	...
	$(document).on('click', '.status', function() {
		...
		listActions('banner', 'status', id, actionType, dTable);
	});
	...
	$(document).on('click', '.delete', function() {
		var id = $(this).data('id');
		var actionType 	= $(this).data('action-type');
		listActions('banner', 'delete', id, actionType, dTable);
	});
	...
	$('.bulkAction').on('click', function() {
		...
		if(selectedIds.length > 0) {
			var actionType = $(this).data('action-type');
			bulkActions('banner', 'bulk-actions', selectedIds, actionType, dTable);
		} else {
