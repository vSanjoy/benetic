...
$(document).ready(function() {
	@if (Route::currentRouteName() == 'admin.teamMember.list')
	// Get list page data
	var getListDataUrl = "{{route('admin.teamMember.list-request')}}";
	...
	var dTable = $('#list-data').on('init.dt', function () {$('#dataTableLoading').hide();}).DataTable({
		...
		columns: [
		@if ($isAllow || in_array('teamMember.change-status', $allowedRoutes) || in_array('teamMember.delete', $allowedRoutes))
			{
				data: 'id',
				orderable: false,
				searchable: false,
		...
		@endif
			...
			{data: 'name', name: 'name'},
			{
				...
				searchable: false,
				render: function ( data, type, row ) {
					return '<img src="'+row.image+'" width="80" alt="" />';
				},
			},
			{data: 'status', name: 'status'},
		@if ($isAllow || in_array('teamMember.edit', $allowedRoutes) || in_array('teamMember.delete', $allowedRoutes))
	...
	$(document).on('click', '.status', function() {
		...
		listActions('teamMember', 'status', id, actionType, dTable);
	});
	...
	$(document).on('click', '.delete', function() {
		var id = $(this).data('id');
		var actionType 	= $(this).data('action-type');
		listActions('teamMember', 'delete', id, actionType, dTable);
	});
	...
	$('.bulkAction').on('click', function() {
		...
		if(selectedIds.length > 0) {
			var actionType = $(this).data('action-type');
			bulkActions('teamMember', 'bulk-actions', selectedIds, actionType, dTable);
		} else {
