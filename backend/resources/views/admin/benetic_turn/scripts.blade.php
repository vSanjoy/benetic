...
$(document).ready(function() {
	@if (Route::currentRouteName() == 'admin.beneticTurn.list')
	// Get list page data
	var getListDataUrl = "{{route('admin.beneticTurn.list-request')}}";
	...
	var dTable = $('#list-data').on('init.dt', function () {$('#dataTableLoading').hide();}).DataTable({
		...
		columns: [
		@if ($isAllow || in_array('beneticTurn.change-status', $allowedRoutes) || in_array('beneticTurn.delete', $allowedRoutes))
			{
				data: 'id',
				orderable: false,
				searchable: false,
		...
		@if ($isAllow || in_array('beneticTurn.edit', $allowedRoutes) || in_array('beneticTurn.delete', $allowedRoutes))
	...
	$(document).on('click', '.status', function() {
		...
		listActions('beneticTurn', 'status', id, actionType, dTable);
	});
	...
	$(document).on('click', '.delete', function() {
		var id = $(this).data('id');
		var actionType 	= $(this).data('action-type');
		listActions('beneticTurn', 'delete', id, actionType, dTable);
	});
	...
	$('.bulkAction').on('click', function() {
		...
		if(selectedIds.length > 0) {
			var actionType = $(this).data('action-type');
			bulkActions('beneticTurn', 'bulk-actions', selectedIds, actionType, dTable);
		} else {
