<?php
/*****************************************************/
# Page/Class name   : BenefitsController
/*****************************************************/
...
class BenefitsController extends Controller
{
    ...
    {
        $data['pageTitle'] = trans('custom_admin.label_benefit_list');
        $data['panelTitle']= trans('custom_admin.label_benefit_list');

        try {
            // Start :: Manage restriction
            $data['isAllow'] = false;
            $restrictions   = AdminHelper::checkingAllowRouteToUser('benefit.');
            ...
            return view('admin.benefit.list', $data);
        } catch (Exception $e) {
    ...
    public function listRequest(Request $request)
    {
        try {
            if ($request->ajax()) {
                ...
                $query = Benefit::whereNull('deleted_at');
                $data = $query->get();
                ...
                return Datatables::of($data, $isAllow, $allowedRoutes)
                        ->addIndexColumn()
                        ->addColumn('page', function ($row) use ($isAllow, $allowedRoutes) {
                            return $row->pageDetails->title;
                        })
                        ...
                        ->addColumn('action', function ($row) use ($isAllow, $allowedRoutes) {
                            $btn = '';
                            if ($isAllow || in_array('benefit.edit', $allowedRoutes)) {
                                $editLink = route('admin.benefit.edit', AdminHelper::customEncryptionDecryption($row->id));                            
    ...
    {
        $data['pageTitle']  = trans('custom_admin.label_add_benefit');
        $data['panelTitle'] = trans('custom_admin.label_add_benefit');
    
        try
        {
            // Breadcrumb
            $data['breadcrumb'] = [
                                    'module_title' => trans('custom_admin.label_benefit_list'),
                                    'module_url' => 'admin.benefit.list'
                                ];
        	if ($request->isMethod('POST'))
        	{
				...
				$validationMessages = array(
                    'cms_page_id.required'  => trans('custom_admin.error_page_id'),
				...
				if ($validator->fails()) {
                    ...
                    if ($image != '') {
                        ...
                        $fileName       = 'benefit_'.strtotime(date('Y-m-d H:i:s')).'.'.$extension;                        
                        $imageResize    = Image::make($image->getRealPath());
                        $imageResize->save(public_path('images/uploads/benefit/' . $fileName));
                    }
                    $details                = new Benefit;
                    $details->cms_page_id   = isset($request->cms_page_id) ? trim($request->cms_page_id,' ') : 0;
                    $details->title         = isset($request->title) ? trim($request->title,' ') : null;
                    $details->image         = $fileName;
                    $details->sort          = AdminHelper::generateSortNumber(new Benefit());
                    if ($details->save()) {
            ...
            $data['cmsPages'] = $cmsPages = Cms::whereIn('id',[3,4,5])->select('id','title')->get();
			return view('admin.benefit.add', $data);
		} catch (Exception $e) {
            $this->generateToastMessage('error', trans('custom_admin.error_something_went_wrong'), false);
            return redirect()->route('admin.benefit.list');
        } catch (\Throwable $e) {
    ...
    {
        $data['pageTitle'] = trans('custom_admin.label_edit_benefit');
        $data['panelTitle']= trans('custom_admin.label_edit_benefit');
        ...
        try
        {
            ...
            $data['details'] = $details = Benefit::where(['id' => $id])->first();
            ...
            if ($request->isMethod('POST')) {
				...
				if ($validator->fails()) {
                    ...
                    if ($image != '') {
                        ...
                        $largeImage = public_path().'/images/uploads/benefit/'.$details->image;
                        @unlink($largeImage);
                        
                        $details->image = $fileName;
                    }
                    $details->cms_page_id   = isset($request->cms_page_id) ? trim($request->cms_page_id,' ') : null;
                    $details->title         = isset($request->title) ? trim($request->title,' ') : null;
                    if ($details->save()) {
    ...
    public function status(Request $request, $id = null)
    {
        ...
        try {
            if ($request->ajax()) {
                $id = AdminHelper::customEncryptionDecryption($id, 'decrypt');
                if ($id != null) {
                    $details = Benefit::where('id', $id)->first();
                    if ($details != null) {
                        if ($details->status == 1) {
                            $details->status = '0';
                            $details->save();
            ...
            if ($request->ajax()) {
                ...
                if (count($selectedIds) > 0) {
                    if ($actionType ==  'active') {
                        Benefit::whereIn('id', $selectedIds)->update(['status' => '1']);
                        
                        $title      = trans('custom_admin.message_success');
                        $message    = trans('custom_admin.success_status_updated_successfully');
                        $type       = 'success';
                    } elseif ($actionType ==  'inactive') {
                        Benefit::whereIn('id', $selectedIds)->update(['status' => '0']);

                        $title      = trans('custom_admin.message_success');
                        $message    = trans('custom_admin.success_status_updated_successfully');
                        $type       = 'success';
                    } else {
                        Benefit::whereIn('id', $selectedIds)->delete();
